...
\together{
\centerline{\Large Overall Notation for EconTeX}
\medskip\medskip

\begin{tabular}{rcl}
    $A$ & - & End-of-period $t$ assets
\\  $B$ & - & Middle-of-period $t$ balances ($=R_{t} K_{t}$); bequests in Ricardian equivalence analysis
\\  $C$ & - & Consumption
\\  $D$ & - & Outstanding government debt/bonds
\\  $e$ & - & Labor effort exerted
\\  $\FFunc$ & - & Production function
\\  $G$ & - & Growth factor for labor income
\\  $H$ & - & Human wealth, Habit Stock
\\  $I$ & - & Investment
\\  $J$ & - & Adjustment costs
...
\\  $N$ & - & Population growth factor
\\  $O$ & - & Net wealth including human wealth
...
\together{
\centerline{Non-Roman characters (mainly Greek)}
\begin{tabular}{rcl}
    $\alpha$   & - & General purpose constant
\\  $\beta$   & - & Discount factor
\\  $\delta$  & - & Depreciation rate
\\  $\daleth$ & - & Depreciation factor = $(1-\delta)$ or $1/(1+\delta)$ or $e^{-\delta}$
...
\together{
\centerline{Operators}
\begin{tabular}{rcl}
    $\PDV_{s}^{t}$ & - & Present discounted value between $s$ and $t$
\\  $\PDV_{t}$     & - & Present discounted value between $t$ and end of horizon
\\  $\Ex$ & - & Expectations
\end{tabular}
}

\together{
\centerline{Commands That Produce Variables}
\begin{tabular}{rcl}
\\ \texttt{$\backslash$Eprem} & - & $\hat{\Risky}$ = $\Risky/\Rfree$
\\ \texttt{$\backslash$eprem} & - & $\hat{\risky}$ = $\risky - \rfree$
\\ \texttt{$\backslash$Estdr} & - & $\Estdr$
\\ \texttt{$\backslash$Evarr} & - & $\Evarr$
\\ \texttt{$\backslash$PopGro} & - & $\PopGro$ 
\\ \texttt{$\backslash$labor} & - & $\labor$ 
\\ \texttt{$\backslash$popGro} & - & $\popGro$ 
\\ \texttt{$\backslash$Risky} & - & $\Risky$ 
\\ \texttt{$\backslash$risky} & - & $\risky$
\\ \texttt{$\backslash$Rfree} & - & $\Rfree$
\\ \texttt{$\backslash$rfree} & - & $\rfree$
\\ \texttt{$\backslash$util} & - & $\uFunc$
\\ \texttt{$\backslash$WMkt} & - & $\WMkt$
\\ \texttt{$\backslash$WHum} & - & $\WHum$
\\ \texttt{$\backslash$WAll} & - & $\WAll$
\\ \texttt{$\backslash$wAll} & - & $\wAll$
\end{tabular}
}
